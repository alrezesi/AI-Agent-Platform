...
async def main():
    ...
    rows = await conn.fetch("SELECT task_id, status, lease_expires_at, EXTRACT(EPOCH FROM lease_expires_at) as lease_epoch FROM tasks WHERE status = 'running'")
    print('RUNNING TASKS:')
    for row in rows:
    ...
    for row in rows:
        print(f'  matched: task={row["task_id"]}, lease={row["lease_expires_at"]}, is_expired={row["is_expired"]}')
